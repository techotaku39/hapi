...
describe('selectMobileSessionHeaderSecondary', () => {
    ...
    it('uses the next enabled and available detail when the model is absent', () => {
        ...
        expect(selectMobileSessionHeaderSecondary({ updatedAt: true, createdAt: true, worktree: true })).toBe('updatedAt')
        expect(selectMobileSessionHeaderSecondary({ worktree: true, fastMode: true })).toBe('worktree')
    })
