    ...
    var body: some View {
        NavigationStack(path: $path) {
            inventory
                ...
                .searchable(text: $query, prompt: "Search drafts and filenames")
                .toolbar {
        ...
        if model.isLoading {
            ...
            List {
                Section {
                    ...
                    Text("Drafts stay here until you use them. The send queue runs automatically.")
                }
            }
            ...
            .overlay {
                if model.state.entries.isEmpty {
                    ContentUnavailableView {
                        ...
                        Text("Keep your next instruction here without interrupting the agent.")
                    } actions: {
                        if let interactor {
                            Button("Write the first draft") {
...
struct ScratchlistEntryRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
                ...
                Button(action: onOpen) {
                    VStack(alignment: .leading, spacing: 5) {
                        ...
                        if !entry.attachments.isEmpty {
                            Label(entry.attachments.map(\.filename).joined(separator: ", "), systemImage: "paperclip")
            ...
            HStack(spacing: 8) {
                ScratchlistAgeLabel(updatedAt: entry.updatedAt)
                    .font(typography.captionFont).foregroundStyle(.secondary).lineLimit(1)
                Spacer(minLength: 0)
                if let interactor {
                    Button(queued ? "Retry removal" : "Take to composer") {
                        if queued {
                            Task { _ = await interactor.queueScratchlistEntry(entry) }
                        } else if interactor.hasComposerDraft {
                            confirmRestore = true
                        } else { restore(.append) }
                    }
                ...
                Menu {
                    if let interactor {
                        Button(queued ? "Retry removal" : "Add to send queue", systemImage: queued ? "arrow.clockwise" : "text.line.first.and.arrowtriangle.forward") {
                            Task { if await interactor.queueScratchlistEntry(entry) { onRestore() } }
                        }
                    }
                    Button("Edit original draft", systemImage: "pencil", action: onEdit).disabled(queued)
            ...
            if let error = interactor?.scratchlistEntryErrors[entry.entryId] {
        ...
        .confirmationDialog("You already have a draft", isPresented: $confirmRestore, titleVisibility: .visible) {
            Button("Append to current input") { restore(.append) }
            Button("Save current input, then take draft") { restore(.parkCurrent) }
            Button("Cancel", role: .cancel) {}
        } message: { Text("Your current text and attachments will not be overwritten.") }
                    ...
                    VStack(alignment: .leading, spacing: 20) {
                        ...
                        if !entry.attachments.isEmpty {
                            ScratchlistAttachmentStrip(attachments: entry.attachments, loader: loader,
                                thumbSize: 88, onOpen: onOpenAttachment, onRemove: nil)
                        }
            ...
            if let editor = model.editor {
                ...
                HStack {
                    PhotosPicker(selection: $pickedItem, matching: .images) {
                        Label("Add photo", systemImage: "photo").frame(minHeight: 44).contentShape(Rectangle())
                    }
                    Button { filePickerOpen = true } label: {
                        Label("Files", systemImage: "doc").frame(minHeight: 44).contentShape(Rectangle())
                    }
                    Spacer()
